...
$.ajax({
    type: "POST",
    url: "https://ws_geral.netwish.com.br/imovel/pesquisar",
    data: {
        pesquisa: pesquisa_usuario
    }
}).done((data) => {
    ...
    if(data.length > 0)
    {
        data.forEach((item) => {
            ...
            //                 <span class="float-left">ID do imóvel: ${item.id}</span>
                    ...
                    <div class="modal fade text-left" id="reservar_imovel_${item.id}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel17" style="display: none;" aria-hidden="true">
                        <div class="modal-dialog modal-lg" role="document">
                            <div class="modal-content">
                                ...
                                <form action="${baseURL}app/Functions/php/reservarImovel.php?imovel_id=${item.id}" method="post" class="reservarImovelAjax">
                                    <div class="modal-body">
                                        <img src="${item.thumb}" alt="${item.post_title}" style="width: 100% !important;">
                                        <b ><h5 style='margin-top:30px;'>${item.post_title}</h5></b>
                                        <p>${item.endereco}</p>
                                        <hr>
                                        <div class="row">
        ...
        $("#imoveis_list").html("<div class='col-lg-6'>NENHUM RESULTADO CORRESPONDE A SUA PESQUISA</div>");
    }
});
